...
public class Layer {
	...
	public float[] genOutputs(float[] data){
	...
	public double[] backpropagation(float[] in, double[] drvCostActivations) {
		...
		for(int j = 0; j < inSize; j++) {
			...
			for(int k = 0; k < outSize; k++) {
				...
				double drvZWeight = in[j];
				
				// Change biases.
				this.gradientBiases[k] = drvCostActivations[k] * drvActivationZ * 0.2;
				this.biases[k] -= drvCostActivations[k] * drvActivationZ * 0.2;
			
				double result = drvCostActivations[k] * drvActivationZ * drvZWeight;
				sum += drvCostActivations[k] * drvActivationZ * this.weights[k][j];
				
				// Change weights.
				this.gradient[k][j] = result * 0.2;
				this.weights[k][j] -= result * 0.2;
			}
